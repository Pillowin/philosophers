...
/*   Updated: 2021/10/15 15:20:33 by agautier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	parse(int argc, char **argv, t_philo *philo)
{
	if (!check_args(argc, argv))
		return (FALSE);
	philo->nb_philo = my_atoi(argv[1]);
	philo->time_to_die = my_atoi(argv[2]);
	philo->time_to_eat = my_atoi(argv[3]);
	philo->time_to_sleep = my_atoi(argv[4]);
	philo->nb_must_eat = UINT8_MAX;
	if (argc == 6)
		philo->nb_must_eat = my_atoi(argv[5]);
	return (TRUE);
}
